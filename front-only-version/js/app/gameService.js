function gameService($timeout, gameServiceConfig) {
  ...
  this.createItemInAFreeSquare = function () {
    ...
    for (var i = 0; i < self._availableSquares.length; i++) {
      if (!self._availableSquares[i].isSquareOccupied) {
        picker.push(i);
      }
    }
    ...
    var valuePicked =
      self._valuePick[Math.floor(Math.random() * self._valuePick.length)];
  ...
  this.globalMove = function (direction, callBackFn) {
    ...
    while (someMoveOccured) {
      ...
      moves++;
      if (this._stepbystepMode) someMoveOccured = false;
    }
  ...
  this.canMove = function (item, direction, canChangeState) {
    ...
    if (!inFrontExists) return true;
    if (inFrontExists && !inFrontHasSameValue) return false;
    if (
      specialPosition &&
      selfCanBeFusionned &&
      inFrontExists &&
    ...
    if (
      selfCanBeFusionned &&
      canInFrontBeFusionned &&
      inFrontExists &&
      inFrontHasSameValue &&
      (!inJumpFrontExists || (inJumpFrontExists && !inJumpFrontHasSameValue))
    ) {
      // case 2 : only one element front
    ...
    if (
      ...
      inJumpFrontExists &&
      inFrontHasSameValue &&
      inJumpFrontHasSameValue
    ) {
      // case 3 : two element front, you don't fusion, because they should fusion before
      return false;
    }
  };
  //#endregion
  ...
  this.getAdjacentItems = function (item, direction) {
    ...
    for (var i = 0; i < itemsLength; i++) {
      ...
      if (testItem.x == item.x && testItem.y == item.y) continue;
      switch (direction) {
        case "left":
          if (
            testItem.x == item.x - 1 &&
            ...
            adjacentItemInTheRightDirection = testItem;
          if (
            testItem.x == item.x - 2 &&
            testItem.y == item.y &&
            !testItem.destroy
          )
            adjacent2ndItemInTheRightDirection = testItem;
          break;
        case "right":
          if (
            testItem.x == item.x + 1 &&
    ...
    return [
      adjacentItemInTheRightDirection,
      adjacent2ndItemInTheRightDirection,
    ];
  };
  //#endregion
